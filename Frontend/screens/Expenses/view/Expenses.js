...
// import returnAPIarr from "../controller/ExpensesController";
import axios from "axios";
import AllExpense from "../model/ExpensesModel";
import returnarr from "../controller/ExpensesController";
...
const Expenses = ({navigation}) => {
    const userzid =12;
    const [s,sets] = useState(10500);
    const [b,setb] = useState(25000);
    const [dataOfExpenses,setDataOfExpenses] = useState([]);
    const [isLoading,setisLoading] = useState(true);
    const [expensesSortOpt,setexpensesSortOpt] = useState("DateO");

    const getExpensesData = async (option) => {
        setisLoading(true);
        try{
            // ${expensesSortOpt}
            const resp = await axios.get(`http://${localHost}/expenseIndiviual${option}/12`).then(function (response){
                // console.warn(response.data[2]);
                setDataOfExpenses(response.data);
            });
        }
        finally{
            setisLoading(false);
            // return(dataOfExpenses);
        }
    }
    
    const barwidth = React.useRef(new Animated.Value(0)).current;
    useEffect(() => {
        getExpensesData("DateO");
        axios.get(`http://${localHost}/getSpent/${userzid}`).then(function (response){
            sets(response.data.spent)
        });
        axios.get(`http://${localHost}/getBudget/${userzid}`).then(function (response){
            setb(response.data.budget)
        });
        // console.warn(s);
        StatusBar.setBackgroundColor(colorCodes.expensesDarkBlue);
        if(value=="Group Expenses"){
            navigation.navigate("GExpenses");
        }
        Animated.spring(barwidth, {
            toValue: screenWidth * (( s/b ) * 0.7),
            bounciness: 20,
            speed: 2,
    ...
    const [value, setValue] = useState('Your expenses');
    ...
    const renderItem = ({ item }) => (
        <Pressable onPress={() => { setSelectedIndex(item); setmodalHeight(screenHeight * 0.5); setVisibleExp(true); }}>
            <View style={{ height: screenHeight * 0.1, width: '100%', borderWidth: 2, marginVertical: 5, padding: 10, backgroundColor: '#d6d6d6', borderColor: '#B2B2B2', borderRadius: 13, flexDirection: 'row', alignItems: 'stretch' }}>
        ...
        <SafeAreaView style={{ flex: 1, backgroundColor: colorCodes.expensesDarkBlue }}>
            <View style={{ height: screenHeight * 0.2 }}>
                <View style={{ flexDirection: 'row', marginLeft: screenWidth * 0.03 }}>
                    {value=="Group Expenses"?navigation.navigate("GExpenses"):null}
                    <DropDownPicker
                ...
                <View style={{ alignItems: 'center' }}>
                    <Text style={{ fontSize: 28, fontWeight: '600', color: 'white' }}>PKR {b}</Text>
                    ...
                    <Pressable onPress={() => {setmodalHeight(screenHeight*0.25);setVisibleB(true); }}>
                        <Text style={{ fontSize: 15, fontWeight: '600', color: 'white' }}>Set your budget</Text>
                    </Pressable>
                </View>
            </View>
            <View style={{ backgroundColor: 'white', height: screenHeight * 0.8, borderTopLeftRadius: 23.0, borderTopRightRadius: 23.0 }}>
                    ...
                    <View style={{ width: screenWidth * 0.74, flexDirection: 'row', alignItems: 'stretch' }}>
                        <View style={{ width: screenWidth * 0.37, flexDirection: 'column', alignItems: 'center', justifyContent: 'space-between' }}>
                            ...
                            <View style={{ flexDirection: 'row' }}>
                                <Pressable onPress={() => { setSortByPriceL(false); setSortByPriceH(false); setSortByDateL(false); setSortByDateO(true); setexpensesSortOpt("DateO");getExpensesData("DateO"); }}>
                                    <View style={{ flexDirection: 'row' }}>
                                        <View style={{ backgroundColor: 'white', borderWidth: 3, borderColor: colorCodes.expensesDarkBlue, padding: 2, height: screenHeight * 0.025, width: screenWidth * 0.047, borderRadius: 80, justifyContent: 'center', alignItems: 'center' }}>
                                            {sortByDateO == true ? (<View style={{ backgroundColor: colorCodes.expensesDarkBlue, borderWidth: 3,borderColor:colorCodes.expensesDarkBlue, padding: 2, height: screenHeight * 0.008, width: screenWidth * 0.01, borderRadius: 80 }}></View>) : (<View></View>)}
                                        </View>
                                        <Text style={{ fontSize: 15, color: colorCodes.expensesDarkBlue, fontWeight: '400' }}>Oldest  </Text>
                                    </View>
                                </Pressable>
                                <Pressable onPress={() => { setSortByPriceL(false); setSortByPriceH(false); setSortByDateO(false); setSortByDateL(true); setexpensesSortOpt("DateL");getExpensesData("DateL"); }}>
                                    <View style={{ flexDirection: 'row' }}>
                                        <View style={{ backgroundColor: 'white', borderWidth: 3, borderColor: colorCodes.expensesDarkBlue, padding: 2, height: screenHeight * 0.025, width: screenWidth * 0.047, borderRadius: 80, justifyContent: 'center', alignItems: 'center' }}>
                                            {sortByDateL == true ? (<View style={{ backgroundColor: colorCodes.expensesDarkBlue, borderWidth: 3,borderColor:colorCodes.expensesDarkBlue, padding: 2, height: screenHeight * 0.008, width: screenWidth * 0.01, borderRadius: 80 }}></View>) : (<View></View>)}
                                        </View>
                        ...
                        <View style={{ width: screenWidth * 0.37, flexDirection: 'column', alignItems: 'center', justifyContent: 'space-between' }}>
                            ...
                            <View style={{ flexDirection: 'row' }}>
                                <Pressable onPress={() => { setSortByDateL(false); setSortByDateO(false); setSortByPriceH(false); setSortByPriceL(true); setexpensesSortOpt("PriceL");getExpensesData("PriceL"); }}>
                                    <View style={{ flexDirection: 'row' }}>
                                        <View style={{ backgroundColor: 'white', borderWidth: 3, borderColor: colorCodes.expensesDarkBlue, padding: 2, height: screenHeight * 0.025, width: screenWidth * 0.047, borderRadius: 80, justifyContent: 'center', alignItems: 'center' }}>
                                            {sortByPriceL == true ? (<View style={{ backgroundColor: colorCodes.expensesDarkBlue, borderWidth: 3,borderColor:colorCodes.expensesDarkBlue, padding: 2, height: screenHeight * 0.008, width: screenWidth * 0.01, borderRadius: 80 }}></View>) : (<View></View>)}
                                        </View>
                                        <Text style={{ fontSize: 15, color: colorCodes.expensesDarkBlue, fontWeight: '400' }}>Lowest  </Text>
                                    </View>
                                </Pressable>
                                <Pressable onPress={() => { setSortByDateL(false); setSortByDateO(false); setSortByPriceL(false); setSortByPriceH(true); setexpensesSortOpt("PriceH");getExpensesData("PriceH"); }}>
                                    <View style={{ flexDirection: 'row' }}>
                                        <View style={{ backgroundColor: 'white', borderWidth: 3, borderColor: colorCodes.expensesDarkBlue, padding: 2, height: screenHeight * 0.025, width: screenWidth * 0.047, borderRadius: 80, justifyContent: 'center', alignItems: 'center' }}>
                                            {sortByPriceH == true ? (<View style={{ backgroundColor: colorCodes.expensesDarkBlue, borderWidth: 3,borderColor:colorCodes.expensesDarkBlue, padding: 2, height: screenHeight * 0.008, width: screenWidth * 0.01, borderRadius: 80 }}></View>) : (<View></View>)}
                                        </View>
                ...
                <View style={{ height: '100%', alignItems: 'stretch', marginHorizontal: screenWidth * 0.02 }}>
                    ...
                    >
                    </FlatList>:<Text style={{fontSize:20,fontWeight:'500',color:'black'}}>Loading..</Text>}
                </View>
                <View style={{ alignItems: 'center' }}>
                        ...
                        <View style={{}}>
                            ...
                            {visible == true ? (<View>
                                ...
                                <View style={{ alignItems: 'flex-start' }}>
                                    ...
                                    {show == true ? (<DateTimePicker value={expenseDate} onChange={onChange} minimumDate={new Date(2022, 1, 1)} maximumDate={new Date(2023,11,1)}></DateTimePicker>) : (<View></View>)}
                                    <View style={{ height: screenHeight * 0.03 }}></View>
                                    ...
                                    <TouchableOpacity onPress={()=>{
                                        ...
                                        if(expensesValueError==""&&expensesTitleError==""&&expensesDescriptionError==""&&expensesAmountError==""&&expensesDateError==""){
                                            ...
                                            axios.post(`http://${localHost}/expenseIndiviualPost`, {
                                                uid: 12,
                                                title: expenseTitle,
                                                expAmount:expenseAmount,
                                                expValue:expensesValue,
                                                expDescrip:expenseDescrip,
                                                expDate:text,
                                              })
                                ...
                                <View style={{ alignItems: 'flex-start', flexDirection: 'column' }}>
                                    <View style={{ flexDirection: 'row', height: screenHeight * 0.08, alignItems: 'flex-end', justifyContent: 'space-between', width: screenWidth * 0.7 }}>
                                        <Text style={{ fontSize: 30, color: colorCodes.expensesDarkBlue, fontWeight: '500' }}>{selectedIndex.exp_title}</Text>
                                        <View style={{ flexDirection: 'column' }}>
                                            <Text style={{ fontSize: 30, color: colorCodes.expensesDarkBlue, fontWeight: '500' }}>Rs. {selectedIndex.exp_amount}</Text>
                                            <Text style={{ fontSize: 15, color: colorCodes.expensesDarkBlue, fontWeight: '500' }}>Date:{selectedIndex.exp_date}</Text>
                                        </View>
                                    </View>
                                    <View style={{ height: screenHeight * 0.36, width: screenWidth * 0.7, alignItems: 'center' }}>
                                        <Text style={{ fontSize: 18, color: colorCodes.expensesDarkBlue, fontWeight: '500', textAlign: 'center' }}>{selectedIndex.exp_description}</Text>
                                    </View>
                                </View>
                            </View>):
                            <View style={{ alignItems: 'flex-start', flexDirection: 'column', justifyContent:'center', alignItems:'center' }}>
                            <Text style={{ fontSize: 30, color: colorCodes.expensesDarkBlue, fontWeight: '500' }}>Set Budget</Text>
                            ...
                            <View style={{height:screenHeight*0.02}}></View>
                            <Pressable onPress={()=>{
                                if(expenseBudget<0||expenseBudget==0||expenseBudget<b){
                                    setExpensesBudgetError("Error");
                                }
                                if(expenseBudget>0&&expenseBudget>b){
