...
import { addCIMMenu, addCache, removeMenu } from '../../../src/Redux/Actions/Create-Itineary-Manual/cim-actions';
...
const ViewParticular = ({navigation}) => {
...
//   const city = 'karachi';
  ...
  const cacheCIMMenu = useSelector((state) => state.cimMenu);
  ...
  useLayoutEffect(()=>{
    ...
    // console.warn(payLoadZ.hotels,payLoadZ.phoneNumbers,payLoadZ.placeImgs);   
    dispatch(addCache(type.toString()+'-cache',payLoadZ));
  },[hotels]);
  
  useEffect(() => {
    setLoaderVisible(true);
    async function fetchData(){
      await fetch(`https://maps.googleapis.com/maps/api/place/textsearch/json?query=${type}+in+${city}&key=${API_KEY}`)
    ...
    if(!cacheCIMMenu.cachez[type]){
        fetchData();
    }
    else{
        // console.warn("  Loading Cahe");
        setHotels(cacheCIMMenu.cachez[type].place.sort((a, b) => b.rating - a.rating));
        setPhoneNumbers(cacheCIMMenu.cachez[type].phoneNumbers);
        setPlaceImgs(cacheCIMMenu.cachez[type].placeImgs)
          setLoaderVisible(false);
    }
    ...
    <Pressable onPress={() => {
      ...
      <View style={{ flex: 1, flexDirection: 'row', backgroundColor: 'white', marginBottom: 10, width: screenWidth * 0.95, paddingHorizontal: 5, paddingVertical: 5, borderRadius: 13.3 }}>
        <View style={{width:screenWidth*0.08,justifyContent:'center',alignItems:'center'}}>
          <Pressable onPress={()=>{
            item['placeTypeTAT'] = type;
            item['placePhoneNumberTAT'] = phoneNumbers[item.place_id];
            item['placeImgTAT'] = placeImgs[item.place_id];
            item['currentPostionPriorityTAT'] = cacheCIMMenu.currentPostionPriorityTAT;
            if(cacheCIMMenu.selected[type].some(obj=>obj.name===item.name)===true){
              dispatch(removeMenu(type.toString()+'R',cacheCIMMenu.selected[type].filter(obj=>obj.name!==item.name)))
            }
            else{
              dispatch(addCIMMenu(type,item));
            }
          }}>
            <View style={{width:25,height:25,borderRadius:13,borderWidth:2,borderColor:colorCodes.expensesDarkBlue,justifyContent:'center',alignItems:'center'}}>
              {
                cacheCIMMenu.selected[type].some(obj=>obj.name===item.name)?
                <View style={{width:15,height:15,borderRadius:10,backgroundColor:colorCodes.expensesDarkBlue}}></View>:
                <View></View>
              }
            </View>
          </Pressable>
